...
class Graph {
    ...
    runDFS(startNodes, direction, maxDepth,
        ...
        if(!(startNodes instanceof Array)) { startNodes = [startNodes]; }
    ...
    setPositionsFromCCs(edgeFilter, nodeFilter) {
        ...
        let perRow = Math.ceil(Math.sqrt(ccsMultiple.length));
        let offset = 500;
        let mid = (offset * (perRow - 1)) / 2;
        let minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;

        _.each(ccsMultiple, function(cc, ccIdx) {
            let x = offset * (ccIdx % perRow) - mid;
            let y = offset * Math.floor(ccIdx / perRow) - mid;
            minX = Math.min(minX, x);
            maxX = Math.max(maxX, x);
            minY = Math.min(minY, y);
            maxY = Math.max(maxY, y);

            let perCCRow = Math.ceil(Math.sqrt(cc.length));
        ...
        // Single node per CC
        let baseX = maxX + offset;
        let baseY = minY;
        let perRowSingle = Math.ceil(Math.sqrt(ccsSingle.length));
        let offsetSingle = 100;
        _.each(ccsSingle, function(cc, ccIdx) {
            if(!cc.length) return;

            let x = baseX + offsetSingle * (ccIdx % perRowSingle);
            let y = baseY + offsetSingle * Math.floor(ccIdx / perRowSingle);
            cc[0].x = x;
            cc[0].y = y;
        });
    }
