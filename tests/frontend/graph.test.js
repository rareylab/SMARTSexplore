...
describe('Edge', () => {
    ...
    it('reassigns source/target from the original values if passed a getNodeFn function', () => {
        ...
        const edge_noGetNodeFn = new Edge(
            nodeABEdgeData,
            {},  // empty defaultMeta
            null
        );
        expect(edge_noGetNodeFn.source).not.toBe(nodeA);
        expect(edge_noGetNodeFn.target).not.toBe(nodeB);
        expect(edge_noGetNodeFn.source).toBe(nodeData.id);
        expect(edge_noGetNodeFn.target).toBe(nodeData2.id);
    })
});
